...
if st.session_state.fwd_data is not None and st.session_state.loc_data is not None:
    if st.button("🏁 SPUSTIT KOMPLETNÍ OPTIMALIZACI"):
        ...
        # --- ZOBRAZENÍ VÝSLEDKŮ (VŠECHNO UVNITŘ TLAČÍTKA) ---
        ...
        res['Poptávka'] = df['demand'] * params['h_cover']
        # Výpočet nepokryté spotřeby (pokud součet zdrojů < požadavek)
        res['Nepokryto'] = (res['Poptávka'] - (res['KGJ'] + res['Kotel'] + res['EK'] + res['Nákup'])).clip(lower=0)

        fig_res = go.Figure()
        colors = {'KGJ': '#FF9900', 'Kotel': '#1f77b4', 'EK': '#2ca02c', 'Nákup': '#d62728', 'Nepokryto': '#000000'}
        
        for c in ['KGJ', 'Kotel', 'EK', 'Nákup', 'Nepokryto']:
            if res[c].sum() > 0.001:
                fig_res.add_trace(go.Bar(x=res['T'], y=res[c], name=c, marker_color=colors[c]))
        
        # Přidáme linku celkové poptávky pro kontrolu
        fig_res.add_trace(go.Scatter(x=res['T'], y=res['Poptávka'], name="Požadované teplo", line=dict(color='black', dash='dot')))

        fig_res.update_layout(barmode='stack', title="Hodinový Dispatch tepla [MW]", hovermode="x unified")
