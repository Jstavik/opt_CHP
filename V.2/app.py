...
if st.session_state.fwd_data is not None and loc_file:
    ...
    # Merge a VYČIŠTĚNÍ NaN hodnot (to způsobovalo tu chybu)
    ...
    if st.button("🏁 SPUSTIT KOMPLETNÍ OPTIMALIZACI"):
        ...
        for t in range(T):
            ...
            h_dem = float(df.iloc[t, 4]) # Sloupec 'Poptávka po teple (MW)'
        ...
        fig_res = go.Figure()
        fig_res.add_trace(go.Scatter(x=res['datetime'], y=res['KGJ'], name="KGJ", stackgroup='one'))
        fig_res.add_trace(go.Scatter(x=res['datetime'], y=res['Kotel'], name="Kotel", stackgroup='one'))
        fig_res.add_trace(go.Scatter(x=res['datetime'], y=res['EK'], name="EK", stackgroup='one'))
